defmodule AdvanceWeb.UserAuth do
  import Plug.Conn
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      AdvanceWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def require_admin_user(conn, _opts) do
    if conn.assigns[:current_user] do
      ...
      if user.role === :admin do
        ...
        conn
        |> put_flash(:error, gettext("You must be an administrator to access this page."))
        |> redirect(to: "/")
        |> halt()
      end
    else
  ...
  def put_session_layout(conn, _opts) do
    conn
    |> put_layout(false)
    |> put_root_layout({AdvanceWeb.LayoutView, :session})
  end
end
